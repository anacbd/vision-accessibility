...
# Tratamento de dependências essenciais com mensagens explicativas
try:
    import cv2
    import numpy as np
    ...
    print("[!] Instale as dependências executando:")
    print("    pip install google-genai opencv-python numpy pillow python-dotenv pyttsx3")
    sys.exit(1)
...
class AccessibilityVisionAssistant:
    ...
    def __init__(
        ...
        print(f"[*] Inicializando cliente Google GenAI com o modelo '{self.model_name}'...")
        ...
        # Inicialização das threads auxiliares
    ...
    def start(self):
        ...
        print("[*] Iniciando threads de inferência e voz...")
    ...
    def _ai_worker_loop(self):
        """
        Thread assíncrona responsável por consumir o frame mais recente,
        codificá-lo e despachá-lo para a API Gemini sem travar a interface.
        """
        print("[*] Thread de Visão Gemini ativa e aguardando frames.")
        while self.is_running:
            try:
                # Aguarda até receber um frame novo
            ...
            try:
                ...
                # Redimensiona o frame para otimizar tempo de upload e latência
                # 640px de largura é ideal para o modelo detectar objetos e texto rapidamente
                ...
                # Converte o frame BGR do OpenCV para JPEG em memória
                success, encoded_image = cv2.imencode(
                    ".jpg", frame_to_send, [int(cv2.IMWRITE_JPEG_QUALITY), 80]
                )
                if not success:
                    print("[-] Falha ao codificar frame para JPEG.")
                    continue

                image_bytes = encoded_image.tobytes()

                # Prompt para guiar a resposta imediata
                prompt_content = [
                    types.Part.from_bytes(data=image_bytes, mime_type="image/jpeg"),
                    "Descreva os elementos essenciais desta cena para uma pessoa cega: ambiente, localização de objetos e textos.",
                ]

                # Chamada oficial da API do Gemini usando o SDK google-genai
                response = self.client.models.generate_content(
                    model=self.model_name,
                    contents=prompt_content,
                    config=types.GenerateContentConfig(
                        system_instruction=SYSTEM_INSTRUCTIONS,
                        temperature=0.2, # Baixa temperatura para descrições factuais e objetivas
                        top_p=0.9,
                    ),
                )
                ...
                # Atualiza a descrição compartilhada com segurança de thread
                with self.lock:
                    self.latest_description = description

                # Exibição limpa no console
                print(f"\n[AI - {elapsed:.2f}s]: {description}")

                # Envia para a fila de áudio caso TTS esteja ativo
                if self.enable_tts:
                    try:
                ...
                err_msg = f"Erro na chamada Gemini: {ex}"
                print(f"[-] {err_msg}")
                with self.lock:
                    self.latest_description = "Falha de conexão com a API Gemini."
            finally:
                with self.lock:
                    self.is_ai_busy = False
    ...
    def _tts_worker_loop(self):
        """Thread de Text-to-Speech para narração contínua e sem bloqueio."""
        print("[*] Thread de Síntese de Voz (TTS) iniciada.")
        engine = None
        try:
            engine = pyttsx3.init()
            engine.setProperty("rate", 185)  # Velocidade de fala ágil para acessibilidade
        except Exception as e:
            print(f"[-] Não foi possível inicializar pyttsx3: {e}")
            return
        ...
        while self.is_running:
            ...
            try:
                ...
                print(f"[-] Erro ao sintetizar áudio: {err}")
            finally:
                self.tts_queue.task_done()

    def _draw_hud(self, frame: np.ndarray) -> np.ndarray:
        """
        Desenha uma interface sobreposta (HUD) moderna e acessível no frame OpenCV.
        Inclui caixa semitransparente de alto contraste e quebra de linhas automática.
        """
        overlay = frame.copy()
        h, w = frame.shape[:2]
        ...
        # Indicador visual de atividade da IA
        ...
        # Painel inferior para a audiodescrição (caixa de alto contraste)
        hud_h = 130
        hud_y = h - hud_h
        ...
        cv2.putText(
            overlay,
            "AUDIODESCRICAO DE ACESSIBILIDADE:",
            (15, hud_y + 24),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (0, 200, 255),
        ...
        # Quebra de linhas do texto (Word-Wrap manual para OpenCV)
        words = text.split()
        lines = []
        current_line = []
        max_chars_per_line = max(35, int(w / 11))
        ...
        # Renderiza até 3 linhas de texto no HUD
        y_text = hud_y + 50
        for line in lines[:3]:
            cv2.putText(
                overlay,
        ...
        # Mescla com transparência suave (alpha blend)
        alpha = 0.85
        return cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0)

    def _video_capture_loop(self):
        """Loop principal de captura de vídeo com OpenCV."""
        print(f"[*] Abrindo webcam padrão (índice {self.camera_index})...")
        cap = cv2.VideoCapture(self.camera_index)

        # Otimizações de buffer da câmera
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        if not cap.isOpened():
            print(f"[-] Não foi possível acessar a câmera no índice {self.camera_index}.")
            print("[!] Verifique as permissões do sistema ou informe outro índice.")
        ...
        print("[*] Pressione 'q' na janela de vídeo para encerrar a aplicação.")
        ...
        try:
            while self.is_running:
                ...
                # Verifica se está na hora de enviar novo frame para a IA
                # Apenas envia se passou o intervalo E a thread de IA não está ocupada
                with self.lock:
                ...
                if not ai_busy and (now - self.last_analysis_timestamp >= self.analysis_interval):
                    ...
                    try:
                        # Limpa qualquer frame pendente e insere o mais recente
                ...
                if key == ord("q") or key == 27:  # 'q' ou tecla Esc
                    print("\n[*] Solicitado encerramento pelo usuário.")
                    break
...
if __name__ == "__main__":
    # Carrega configurações personalizadas via variáveis de ambiente ou usa defaults
    camera_idx = int(os.getenv("CAMERA_INDEX", "0"))
    model = os.getenv("GEMINI_MODEL", "gemini-3.6-flash")
    interval = float(os.getenv("ANALYSIS_INTERVAL_SECONDS", "3.0"))
    tts_enabled = os.getenv("ENABLE_VOICE_NARRATION", "True").lower() in ("true", "1", "yes")

    print("=" * 70)
    print("   ASSISTENTE DE ACESSIBILIDADE EM TEMPO REAL - GEMINI VISION AI")
    print("=" * 70)
    print(f" -> Modelo:             {model}")
    print(f" -> Intervalo de IA:    {interval} segundos")
    print(f" -> Câmera (índice):    {camera_idx}")
    print(f" -> Narração por Voz:   {'Ativada' if tts_enabled and HAVE_TTS else 'Desativada (ou pyttsx3 ausente)'}")
    print("=" * 70)
